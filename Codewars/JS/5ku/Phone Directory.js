function phone(string, num) {
    ...
    let formatted_entries = {},
        duplicates = [];
    (name_regex = /<(.*)>/), (phone_regex = /\+(\d{1,2}-\d{3}-\d{3}-\d{4})/);
    result = '';
    entries.forEach((a) => {
        let phone = a.match(phone_regex)[1],
            name = a.match(name_regex)[1],
            address = a
                .replace(name_regex, '')
                .replace(phone_regex, '')
                .replace(/[/:,!@#$%^&*;?]/g, '')
                ...
                .replace(/  /g, ' ')
                .trim();
        if (Object.keys(formatted_entries).includes(phone))
            duplicates.push(phone);
        formatted_entries[phone] = [name, address];
    });
    if (duplicates.includes(num)) return 'Error => Too many people: ' + num;
    if (!Object.keys(formatted_entries).includes(num))
        return 'Error => Not found: ' + num;
    result += 'Phone => ' + num;
    result += ', Name => ' + formatted_entries[num][0];
    result += ', Address => ' + formatted_entries[num][1];
    return result;
    // console.log(entries);
}
